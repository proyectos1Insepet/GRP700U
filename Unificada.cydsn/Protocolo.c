...
* @return 1 si detecta las dos posiciones 0 si no
*
*/
uint8 ver_pos (void){
uint8 x;
...
/**
* detener
* @brief Envía un stop a la posición seleccionada
    ...
    if(Surtidor_GetRxBufferSize()<=39){
        ...
        if((rx_venta[0]==0xFF)&&(rx_venta[2]==0xF8)&&((rx_venta[4]&0x0F)==(pos-1))){
            ...
            if((rx_venta[x]==0xF0)&&(x<=38)){								
				if(val==lado.a.dir){                 /// <B>Almacena los datos del lado a </B>
					write_eeprom(457,rventa.dinero); /// write_eeprom(457,rventa.dinero);
					write_eeprom(466,rventa.volumen);/// write_eeprom(466,rventa.volumen);
					write_eeprom(475,rventa.ppu);    /// write_eeprom(475,rventa.ppu);
				}				
				else{                               /// <B>Almacena los datos del lado b </B>
					write_eeprom(482,rventa.dinero);/// write_eeprom(482,rventa.dinero);
					write_eeprom(500,rventa.ppu);    /// write_eeprom(500,rventa.ppu);
					write_eeprom(491,rventa.volumen); /// write_eeprom(491,rventa.volumen);				
				}
                return 1;
            }
            else{
